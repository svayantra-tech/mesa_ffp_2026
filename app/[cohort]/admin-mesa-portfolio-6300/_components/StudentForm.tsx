...
export default function StudentForm({ cohort, student, brands }: Props) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const url = editing
        ? `/api/admin/students/${student!.id}?cohort=${cohort}`
        : `/api/admin/students?cohort=${cohort}`
  ...
  async function remove() {
    ...
    setBusy(true)
    const res = await fetch(`/api/admin/students/${student!.id}?cohort=${cohort}`, { method: 'DELETE' })
    if (res.ok) {
